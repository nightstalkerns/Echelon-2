...
class DB_B3 {
	...
	/**
	 * Add the mapconfig information the mapconfig table
	 *
	 * @param string $mapname - map name
	 * ...
	 * @return bool
	 */
	function addMapconfig($mapname, $capturelimit, $g_suddendeath, $g_gear, $g_gravity, $g_friendlyfire, $g_knockback, $startmessage, $skiprandom, $datelastadd, $timelimit) {
		$time = time();
		$query = "INSERT INTO mapconfig VALUES(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$stmt = $this->mysql->prepare($query) or die('Database Error');
		$stmt->bind_param('siisiiisisi', $mapname, $capturelimit, $g_suddendeath, $g_gear, $g_gravity, $g_friendlyfire, $g_knockback, $startmessage, $skiprandom, $datelastadd, $timelimit);
		$stmt->execute();
	 ...
	 * Deletes a mapconfig
	 *
	 * @param string $map_id - id of mapconfig to deactive
	 * @return bool
	 */
	function delMapconfig($map_id) {
		$query = "DELETE FROM mapconfig WHERE id = ? LIMIT 1";
		$stmt = $this->mysql->prepare($query) or die('Database Error');
	...
	function editMapconfig($id, $mapname, $capturelimit, $g_suddendeath, $g_gear, $g_gravity, $g_friendlyfire, $g_knockback, $startmessage, $skiprandom, $datelastadd, $timelimit) {
		$query = "UPDATE mapconfig SET mapname = ?, capturelimit = ?, g_suddendeath = ?, g_gear = ? , g_gravity = ? , g_friendlyfire = ? , g_knockback = ?, startmessage = ?, skiprandom = ?, datelastadd = ?, timelimit = ? WHERE id = ? LIMIT 1";
		$stmt = $this->mysql->prepare($query) or die('Database Error');
		$stmt->bind_param('siisiiisisii', $mapname, $capturelimit, $g_suddendeath, $g_gear, $g_gravity, $g_friendlyfire, $g_knockback, $startmessage, $skiprandom, $datelastadd, $timelimit, $id);
		$stmt->execute();
